...
public class ConsumerClient{
    ...
    public int postForStatusCode(String path, String body, Header header, ContentType mimeType) throws IOException {
        int code = Request.Post(url + encodePath(path))
                .addHeader(TESTREQHEADER, TESTREQHEADERVALUE)
                .addHeader(header)
                .bodyString(body, mimeType)
    ...
    public Map postForJSonResponse(String path, String body, Header header, ContentType mimeType) throws IOException {
        String respBody = Request.Post(url + encodePath(path))
                .addHeader(TESTREQHEADER, TESTREQHEADERVALUE)
                .addHeader(header)
                .bodyString(body, mimeType)
                .execute().returnResponse().getEntity().toString();
        return jsonToMap(respBody);
    }
